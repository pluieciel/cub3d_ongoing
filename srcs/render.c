...
void	change_image_color(t_data *game, t_image *img)
{
	...
	while (i < img->w)
	{
		...
		while (j < img->h)
		{
			...
			{
				gray = t & 0xFF;
				target_r = (game->hud_color >> 16) & 0xFF;
				target_g = (game->hud_color >> 8) & 0xFF;
				target_b = game->hud_color & 0xFF;
				new_r = (target_r * gray) / 255;
				new_g = (target_g * gray) / 255;
				new_b = (target_b * gray) / 255;
				new_t = (new_r << 16) | (new_g << 8) | new_b;
...
int	render(t_data *game)
{
	...
	if (game->win_ptr != NULL && (current_time - game->time) > 1000 / FPS)
	{
		...
		render_image(game, &game->img_hud_health, 20, WIN_H
			- game->img_hud_health.h - 20);
		render_image(game, &game->img_hud_one, 20 + game->img_hud_health.w + 10,
			WIN_H - game->img_hud_one.h - 20);
		render_image(game, &game->img_hud_zero, 20 + game->img_hud_health.w + 20
			+ game->img_hud_one.w, WIN_H - game->img_hud_zero.h - 20);
		render_image(game, &game->img_hud_zero, 20 + game->img_hud_health.w + 25
			+ game->img_hud_one.w + game->img_hud_zero.w, WIN_H
			- game->img_hud_zero.h - 20);
		render_image(game, &game->img_hud_divider, 40 + game->img_hud_health.w
			+ 25 + game->img_hud_one.w + game->img_hud_zero.w
			+ game->img_hud_zero.w, WIN_H - game->img_hud_zero.h - 20);
		render_image(game, &game->img_hud_full_suit, 60 + game->img_hud_health.w
			+ 25 + game->img_hud_one.w + game->img_hud_zero.w
			+ game->img_hud_zero.w, WIN_H - game->img_hud_zero.h - 25);
		render_image(game, &game->img_hud_one, 190, WIN_H - game->img_hud_one.h
			- 20);
		render_image(game, &game->img_hud_zero, 208, WIN_H
			- game->img_hud_zero.h - 20);
		render_image(game, &game->img_hud_zero, 230, WIN_H
			- game->img_hud_zero.h - 20);
		render_image(game, &game->img_hud_flash_full, WIN_W
			- game->img_hud_flash_full.w - 20, 20);
